        ...
        <div v-if="drawerOpen" class="event-drawer">
          <div class="drawer-header">
            ...
            <div class="search-bar">
              ...
              <div class="search-loading-bar" :class="{ active: search.isLoading }">
                <div class="loading-track" />
              </div>
              <Transition name="menu-fade">
                <div v-if="search.showDropdown && search.query.trim()" class="search-dropdown">
                  <template v-if="search.results.length > 0">
                    <div
...
import { useStarlinkSearch } from '@/composables/useStarlinkSearch'
...
import { allEvents as historyEvents, backendAvailable, loadError, loadEvents } from '@/data/events'
...
// 搜索逻辑全部封装在 composable 中: 防抖 / RAG 调用 / timer 清理 / 卸载保护
const search = useStarlinkSearch()
...
function handleSearchSelect(id: string) {
  // eventById 在 composable 内部仍会校验; 这里仅负责触发导航 + 关闭下拉
  search.reset()
  goToEvent(id)
}

function handleSearchEnter() {
  // 委托给 composable 处理: 若有结果则跳转第一条, 否则保持下拉展开
  const id = search.onEnter()
  if (id) {
    goToEvent(id)
  }
}
